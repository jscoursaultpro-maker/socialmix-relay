...
export async function verifySupabaseJWT(token) {
  ...
  // Fail-safe : refuser les tokens de test en production
  if (token.startsWith('test:') && process.env.NODE_ENV !== 'test') {
    console.warn(`[Auth] Rejected test:* token in non-test env (NODE_ENV=${process.env.NODE_ENV})`);
    throw new AuthError('TOKEN_INVALID', 'test_token_in_production');
  }
  ...
  if (process.env.NODE_ENV === 'test' && token.startsWith('test:')) {
    ...
    if (payload.aud && payload.aud !== AUDIENCE) {
      throw new AuthError('TOKEN_AUDIENCE', 'JWT audience mismatch');
    }
    if (payload.iss && payload.iss !== ISSUER) {
      throw new AuthError('TOKEN_INVALID', 'JWT issuer mismatch');
    }
